...
const SPREADSHEET_ID = '1ycvegNB6trRt632TDEBnRGrXqDU_NoYRgnB6woSRnls'; 
...
// ----------------------------------------------------
function doGet(e){
  // 1. ดึงอีเมลอย่างปลอดภัย ป้องกัน Error กรณีเข้าใช้งานแบบไม่ระบุตัวตน
  let rawEmail = '';
  try {
    rawEmail = Session.getActiveUser().getEmail();
  } catch (err) {
    console.log('Cannot get user email');
  }
  
  // ถ้าไม่มีอีเมล (ไม่ได้ล็อกอิน) ให้ใช้ guest ชั่วคราว หรือบังคับให้ล็อกอิน
  const userEmail = rawEmail ? rawEmail.trim().toLowerCase() : 'guest@anonymous.com';
  ...
  if(page){
    switch(page){
      case 'superadmin':
        if(!permissions.isSuperAdmin) return HtmlService.createHtmlOutput('Access Denied. You do not have Super Admin rights.');
        return renderPage('SuperAdmin_View','Super Admin',templateData);
      case 'admin':
        if(!permissions.canEditLessons && !permissions.canEditQuizzes) return HtmlService.createHtmlOutput('Access Denied. You do not have Admin rights.');
  ...
  // หากไม่ได้ระบุ page ให้ไปหน้า Student_View เป็นค่าเริ่มต้น
  templateData.studentData = getStudentData(userEmail);
  return renderPage('Student_View','E-Learning Platform',templateData);
}
...
function submitExternalProof(data) {
  ...
  try {
    let fileUrl = '';
    if (data.imageData) {
      // ⭐ ข้อควรระวัง: กรุณาเปลี่ยน ID นี้ให้เป็น ID โฟลเดอร์ใน Google Drive ของคุณเอง
      // ไม่เช่นนั้นหากสิทธิ์ไม่ถูกต้อง ระบบจะ Error และหน้าจอจะค้าง
      const folderId = "1FK6B_zx6xtP8EpDT-Qdnc50H10hs5pH9"; 
      const folder = DriveApp.getFolderById(folderId);
      
      // JAVIS FIXED: ใส่ try-catch กัน Error กรณีองค์กรปิดกั้นการแชร์ไฟล์ภายนอก
      try {
        folder.setSharing(DriveApp.Access.ANYONE_WITH_LINK, DriveApp.Permission.VIEW);
      } catch (shareErr) {
        console.warn("ไม่สามารถตั้งค่า Sharing ได้ อาจติดนโยบายองค์กร:", shareErr);
      }
    ...
    return { status: 'error', message: 'การบันทึกภาพล้มเหลว โปรดตรวจสอบ Folder ID ของคุณ (Error: ' + e.message + ')' };
  }
}
...
function updateUserProfile(data) {
  ...
  try {
    ...
    if (data.imageData) {
      const folderId = "1PW82WGcvSSJliDjOLGoOSVzGUv_qBrPe"; 
      const folder = DriveApp.getFolderById(folderId);
